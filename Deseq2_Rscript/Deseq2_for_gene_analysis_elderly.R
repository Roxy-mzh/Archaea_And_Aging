...
gene_data <- as.matrix(read.csv("unnormalized_annotated_genes_elderly.csv", header = TRUE, sep = ",", dec = ".", row.names="gene_id"))
View(gene_data)

#Import the metadata file 

metadata <- read.csv("metadata_elderly.csv", header = TRUE, sep = ",", dec = ".", row.names="Sample_ID")
View(metadata)
...
write.csv(normalized_counts, file="normalized_counts_elderly.csv")
...
#The count of 10 is a reasonable choice for bulk RNA-seq (min number of reads) #number of methane producers in elderly=36 
...
write.csv(as.data.frame(resOrdered), file="methane_production_Deseq2_elderly_results.csv")
