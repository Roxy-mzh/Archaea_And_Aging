...
gene_data <- as.matrix(read.csv("unnormalized_annotated_genes_centenarians.csv", header = TRUE, sep = ",", dec = ".", row.names="gene_id"))
View(gene_data)

#Import the metadata file 

metadata <- read.csv("metadata_centenarians.csv", header = TRUE, sep = ",", dec = ".", row.names="Sample_ID")
View(metadata)
...
write.csv(normalized_counts, file="normalized_counts_centenarians.csv")

#OR
dds <- DESeq(dds)
sizeFactors(dds)
normalized_counts_DESeq <- counts(dds, normalized=TRUE)
write.csv(normalized_counts_DESeq, file="normalized_counts_centenarians.csv")
...
#Pre-filtering
#present in at least 3 samples (A recommendation for the minimal number of samples is to specify the smallest group size: in my case number of high methane
#The count of 10 is a reasonable choice for bulk RNA-seq (min number of reads) #number of methane producers in centenarians=20 
...
write.csv(as.data.frame(resOrdered), file="methane_production_Deseq2_centenarians_results.csv")
